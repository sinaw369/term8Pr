...
func GetDashBoard3Content(ctx *context.Context) (types.Panel, error) {
	...
	line.SetSeriesOptions(
		charts.MLNameTypeItem{Name: "averageValue", Type: "average"},
		charts.LineOpts{Smooth: true},
		charts.MLStyleOpts{Label: charts.LabelTextOpts{Show: true, Formatter: "{a}: {b}"}},
	)
	line.SetGlobalOptions(
		charts.YAxisOpts{Name: "SearchTime(ns)", SplitLine: charts.SplitLineOpts{Show: false}},
		charts.XAxisOpts{Name: "NumberOfElements"})
	line.Width = "250px"
	line.Height = "250px"

	ecbox := components.Box().WithHeadBorder().SetHeader("QueryTimeComparisonHashTable vs binarySearch").SetBody(echart.SetContent(line).GetContent()).GetContent()

	bar := charts.NewBar()
	bar.SetGlobalOptions(
		charts.YAxisOpts{AxisLabel: charts.LabelTextOpts{Formatter: "{value} item/day"}},
	)
	bar.AddXAxis([]string{"shirt", "jeans", "sweatpants", "sock", "jacket", "cardigan"}).
		AddYAxis("merchantA", randInt(), charts.BarOpts{YAxisIndex: 0}).
		AddYAxis("merchantB", randInt(), charts.BarOpts{YAxisIndex: 1})
	bar.ExtendYAxis(charts.YAxisOpts{AxisLabel: charts.LabelTextOpts{Formatter: "{value} item/month"}})
	bar.Width = "250px"
	bar.Height = "250px"

	ecbox2 := components.Box().WithHeadBorder().SetHeader("Bar-multipleYAxes").SetBody(echart.SetContent(bar).GetContent()).GetContent()
	...
	ecbox3 := components.Box().WithHeadBorder().SetHeader("Bar3D-sampleGraph").SetBody(echart.SetContent(bar3d).GetContent()).GetContent()
	...
	ecbox4 := components.Box().WithHeadBorder().SetHeader("BoxPlot-sampleGraph").SetBody(echart.SetContent(bp).GetContent()).GetContent()
	...
	ecboxCol5 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("EffectScatter-sampleGraph").SetBody(echart.SetContent(es).GetContent()).GetContent()).GetContent()
	...
	ecboxCol6 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Funnel-sampleGraph").SetBody(echart.SetContent(funnel).GetContent()).GetContent()).GetContent()

	gauge := charts.NewGauge()
	m := make(map[string]interface{})
	m["sample"] = rand.Intn(50)
	gauge.Add("gauge", m)
	gauge.Width = "250px"
	gauge.Height = "250px"

	ecboxCol7 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Gauge-sampleGraph").SetBody(echart.SetContent(gauge).GetContent()).GetContent()).GetContent()
	...
	ecboxCol8 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Geo-sampleGraph").SetBody(echart.SetContent(geo).GetContent()).GetContent()).GetContent()
	...
	ecboxCol9 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Graph-sampleGraph").SetBody(echart.SetContent(graph).GetContent()).GetContent()).GetContent()
	...
	ecboxCol10 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("HeatMap-sampleGraph").SetBody(echart.SetContent(hm).GetContent()).GetContent()).GetContent()
	...
	ecboxCol12 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Line3D-sampleGraph").SetBody(echart.SetContent(line3d).GetContent()).GetContent()).GetContent()
	...
	ecboxCol13 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Liquid-sampleGraph").SetBody(echart.SetContent(liquid).GetContent()).GetContent()).GetContent()
	...
	ecboxCol14 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Map-sampleGraph").SetBody(echart.SetContent(mc).GetContent()).GetContent()).GetContent()
	...
	ecboxCol15 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Pie-sampleGraph").SetBody(echart.SetContent(pie).GetContent()).GetContent()).GetContent()
	...
	radar.Add("tehran", radarDataBJ)
	radar.Width = "250px"
	radar.Height = "250px"

	ecboxCol16 := colComp.SetSize(types.SizeMD(3)).SetContent(components.Box().WithHeadBorder().
		SetHeader("Radar-sampleGraph").SetBody(echart.SetContent(radar).GetContent()).GetContent()).GetContent()
...
var (
	nameItems = []string{"shirt", "jeans", "sweatpants", "sock", "jacket", "cardigan"}
	...
	mapData = map[string]float32{
		"beijing":          float32(rand.Intn(150)),
		"shanghai":         float32(rand.Intn(150)),
		"shenzhen":         float32(rand.Intn(150)),
		"liaoningProvince": float32(rand.Intn(150)),
		"qingdao":          float32(rand.Intn(150)),
		"shanxi":           float32(rand.Intn(150)),
		"shanxiProvince":   float32(rand.Intn(150)),
		"urumqi":           float32(rand.Intn(150)),
		"qiqihar":          float32(rand.Intn(150)),
	}
